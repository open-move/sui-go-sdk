...
// SendObjects demonstrates how to send objects (single and multiple)
func SendObjects(ctx context.Context, client *graphql.Client, kp keypair.Keypair, sender, recipient string, gasPrice uint64, gasCoinRef *types.ObjectRef) *types.ObjectRef {
	fmt.Println()
	fmt.Println("=== Example 3: Send Objects ===")

	gasBudget := uint64(15000000)
	...
	// For this example, we'll demonstrate splitting coins and transferring them
	fmt.Println("Demonstrating coin split and transfer transaction...")
	...
	// Split into 3 coins and transfer them to the recipient
	amounts := []transaction.Argument{
		tb.PureU64(100000),
		tb.PureU64(100000),
		tb.PureU64(100000),
	}
	splitResults := tb.SplitCoins(transaction.SplitCoins{
		Coin:    tb.Gas(),
		Amounts: amounts,
	})

	// Transfer all split coins to the recipient
	recipientArg := tb.PureAddress(recipient)
	tb.TransferObjects(transaction.TransferObjects{
		Objects: splitResults,
	...
	fmt.Printf("Split and Transfer transaction built successfully!\n")
	fmt.Printf("  BCS size: %d bytes\n", len(buildResult.TransactionBytes))
	fmt.Printf("  Creating and transferring 3 coins of 100000 MIST each\n")
